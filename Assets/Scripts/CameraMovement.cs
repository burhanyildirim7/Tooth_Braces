...
public class CameraMovement : MonoBehaviour
{
    ...
    {
        transformParent.DOMove(parentPos + Vector3.right * (Input.mousePosition.x - Screen.width / 2) * .002f, 1.25f);
        transformParent.DORotateQuaternion(Quaternion.Euler(Vector3.up * (Input.mousePosition.x - Screen.width / 2) * -.013f), 1.25f);
    }
